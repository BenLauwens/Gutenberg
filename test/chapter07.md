...
```math
S=2\textup{π}\int_{t=a}^{t=b}\left|y\right|\mathrm{d}\kern-0.5pt s=2\textup{π}\int_a^b\left|g\left(t\right)\right|\sqrt{\left(f^\prime\left(t\right)\right)^2+\left(g^\prime\left(t\right)\right)^2} \,\mathrm{d}\kern-0.5pt t\,.
```

If the rotation is about the ``y``-axis, then the area is

```math
S=2\textup{π}\int_{t=a}^{t=b}\left|x\right|\mathrm{d}\kern-0.5pt s=2\textup{π}\int_a^b\left|f\left(t\right)\right|\sqrt{\left(f^\prime\left(t\right)\right)^2+\left(g^\prime\left(t\right)\right)^2} \,\mathrm{d}\kern-0.5pt t\,.
```
...
!!! example
	...
	The curve is symmetric about both coordinate axes. The entire surface will be generated by rotating the upper half of the curve; in fact, we need only rotate the first quadrant part and multiply by ``2``. The first quadrant part of the curve corresponds to ``0\le t\le\frac{\textup{π}}{2}``. We have
	...
	```math
	S = 2 \times 2\textup{π}\int_0^\frac{\textup{π}}{2}a\sin^3 t\,3a\cos t\sin t\mathrm{d}\kern-0.5pt t=\frac{12}{5}\textup{π} a\ \textrm{square units.}
	```
...
!!! example
	...
	```math
	x=a\cos s\quad\textrm{and}\quad y=b\sin s\quad\textrm{for } 0\le s\le 2\textup{π}\,,\quad\textrm{where }a&gt;0, b&gt;0\,.
	```
	...
	```math
	\begin{aligned}
	A&=-\int_0^{2\textup{π}}b\sin s\left(-a\sin s\right)\,\mathrm{d}\kern-0.5pt s=\frac{ab}{2}\int_0^{2\textup{π}}\left(1-\cos 2s\right)\,\mathrm{d}\kern-0.5pt s\\
	&=\left.\frac{ab}{2}s\right|_0^{2\textup{π}}-\left.\frac{ab}{4}\sin 2s\right|_0^{2\textup{π}}=\textup{π} ab\ \textrm{square units.}
	\end{aligned}
	```
...
Figure("", "Polar coordinates.") do
	...
	Drawing(width=10scale, height=10scale) do
		...
		latex("\\frac{\\textup{π}}{12}", x=xmid+4scale*cos(pi/12)+5, y=ymid-4scale*sin(pi/12)-10, width=font_x, height=2font_y)
		latex("\\frac{\\textup{π}}{6}", x=xmid+4scale*cos(pi/6)+4, y=ymid-4scale*sin(pi/6)-12, width=font_x, height=2font_y)
		latex("\\frac{\\textup{π}}{4}", x=xmid+4scale*cos(pi/4)+2, y=ymid-4scale*sin(pi/4)-14, width=font_x, height=2font_y)
		latex("\\frac{\\textup{π}}{3}", x=xmid+4scale*cos(pi/3), y=ymid-4scale*sin(pi/3)-18, width=font_x, height=2font_y)
		latex("\\frac{5\\textup{π}}{12}", x=xmid+4scale*cos(5pi/12)-2, y=ymid-4scale*sin(5pi/12)-22, width=font_x, height=2font_y)
		...
		latex("\\left(3,\\frac{\\textup{π}}{6}\\right)_P", x=xmid+3scale*cos(pi/6)-20, y=ymid-3scale*sin(pi/6)-25, width=3font_x, height=2font_y)
	end
end
```

Unlike rectangular coordinates, the polar coordinates of a point are not unique. The polar coordinates ``\left(r,\theta_1\right)_P`` and ``\left(r,\theta_2\right)_P`` represent the same point provided ``\theta_1`` and ``\theta_2`` differ by an integer multiple of ``2\textup{π}``. For instance, the polar coordinates

```math
\left(3,\frac{\textup{π}}{6}\right)_P=\left(3,\frac{13\textup{π}}{6}\right)_P=\left(3,\frac{-11\textup{π}}{6}\right)_P=\left(-3,\frac{7\textup{π}}{6}\right)_P=\left(-3,\frac{-5\textup{π}}{6}\right)_P
```
...
!!! theorem
	...
	In particular, ``\psi=\frac{\textup{π}}{2}`` if ``f^\prime\left(\theta\right)=0``. If ``f\left(\theta_0\right)=0`` and the curve has a tangent line at ``\theta_0``, then that tangent line has equation ``\theta=\theta_0``.
...
!!! proof
	...
	{cell=chap display=false output=false}
	```julia
	Figure("", tex("\\psi+\\theta=\\phi+\\textup{π}\\,.")) do
		scale = 50
		Drawing(width=2.5scale, height=2.5scale) do
			defs() do
				marker(id="arrow", markerWidth="6", markerHeight="6", refX="3", refY="3", orient="auto") do
	...
	```math
	\begin{aligned}
	\tan\left(\psi+\theta\right)&=\tan\left(\phi+\textup{π}\right)=\tan\left(\phi\right)\\
...
```math
\mathrm{d}\kern-0.5pt A=r^2\textup{π}\frac{1}{2\textup{π}}\,\mathrm{d}\kern-0.5pt\theta=\frac{1}{2}r^2\,\mathrm{d}\kern-0.5pt\theta=\frac{1}{2}\left(f\left(\theta\right)\right)^2\,\mathrm{d}\kern-0.5pt\theta\,.
```
...
!!! example
	...
	Figure("", "The area between two polar curves.") do
		...
		Drawing(width=2.5scale, height=3scale) do
			...
			latex("\\frac{\\textup{π}}{4}", x=xmid+1scale, y=ymid-1.25scale, width=font_x, height=2font_y)
		end
	end
	```
	...
	Thus, ``\sin\theta=\cos\theta`` and ``\theta=\frac{\textup{π}}{4}``. The required area is

	```math
	\begin{aligned}
	A&=\frac{1}{2}\int_0^\frac{\textup{π}}{4}2\sin^2\theta\,\mathrm{d}\kern-0.5pt \theta+\frac{1}{2}\int_\frac{\textup{π}}{4}^\frac{\textup{π}}{2}2\sin2\theta\,\mathrm{d}\kern-0.5pt \theta\\
	&=\int_0^\frac{\textup{π}}{4}\frac{1-\cos2\theta}{2}\,\mathrm{d}\kern-0.5pt \theta-\left.\frac{1}{4}\cos2\theta\right|_\frac{\textup{π}}{4}^\frac{\textup{π}}{2}\\
	&= \frac{\textup{π}}{8}-\left.\frac{1}{4}\sin2\theta\right|_0^\frac{\textup{π}}{4}+\frac{1}{4}=\frac{\textup{π}}{8}-\frac{1}{4}+\frac{1}{4}=\frac{\textup{π}}{8}\ \textrm{square units.}
	\end{aligned}
	```
...
!!! example
	...
	The total length is twice the length from ``\theta=0`` to ``\theta=\textup{π}``. See figure.

	Since ``r^\prime\left(\theta\right)=-a\sin\theta``, the arc length is

	```math
	\begin{aligned}
	s&=2\int_0^\textup{π}\sqrt{a^2\sin^2\theta+a^2\left(1+\cos\theta\right)}\,\mathrm{d}\kern-0.5pt \theta\\
	&=2\int_0^\textup{π}\sqrt{2a^2+a^2\cos\theta}\,\mathrm{d}\kern-0.5pt \theta\quad\textrm{but }1+\cos\theta=2\cos^2\left(\frac{\theta}{2}\right)\\
	&= 2\sqrt 2a\int_0^\textup{π}\sqrt{2\cos^2\left(\frac{\theta}{2}\right)}\,\mathrm{d}\kern-0.5pt \theta\\
	&= 4a\int_0^\textup{π}\cos\left(\frac{\theta}{2}\right)\,\mathrm{d}\kern-0.5pt \theta=\left.8a\sin\left(\frac{\theta}{2}\right)\right|_0^\textup{π}=8a\ \textrm{units.}
	\end{aligned}
	```
